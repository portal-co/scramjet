...
export class ScramjetServiceWorkerRuntime {
	...
	constructor(public client: ScramjetClient) {
		const handler = (cevent: MessageEvent) => {
			self.removeEventListener("connect", handler);
		...
		self.addEventListener("connect", handler);
	}
